...
def crop_bbox(feats, bbox, HH, WW=None, backend='cudnn'):
    ...
    X = tensor_linspace(x0, x1, steps=WW).view(N, 1, WW).expand(N, HH, WW).to(device=feats.device)
    Y = tensor_linspace(y0, y1, steps=HH).view(N, HH, 1).expand(N, HH, WW).to(device=feats.device)
