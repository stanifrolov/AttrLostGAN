...
class ResnetGenerator128(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3, num_dcolor=None, num_bgcolor=None, num_dstyle=None):
        ...
        self.dcolor_embedding = nn.Embedding(num_dcolor, 180)
        self.bgcolor_embedding = nn.Embedding(num_bgcolor, 180)
        self.dstyle_embedding = nn.Embedding(num_dstyle, 180)

        self.attribute_features = nn.Sequential(nn.Linear(540, 360),
                                                nn.ReLU(),
                                                nn.Linear(360, 180),
                                                nn.ReLU(),
                                                nn.Linear(180, 180))
    ...
    def forward(self, z, bbox, z_im=None, y=None, dc=None, bgc=None, ds=None):
        ...
        dcolor_embedding = self.dcolor_embedding(dc)
        bgcolor_embedding = self.bgcolor_embedding(bgc)
        dstyle_embedding = self.dstyle_embedding(ds)

        z = z.view(b * o, -1)
        label_embedding = label_embedding.view(b * o, -1)
        dcolor_embedding = dcolor_embedding.view(b * o, -1)
        bgcolor_embedding = bgcolor_embedding.view(b * o, -1)
        dstyle_embedding = dstyle_embedding.view(b * o, -1)

        attr_feat = self.attribute_features(torch.cat((dcolor_embedding, bgcolor_embedding, dstyle_embedding), dim=1))
